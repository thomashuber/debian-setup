...
if [[ ! -h /etc/apt/apt.conf.d/100update ]]; then
  ((ERRORS_NUMBER_SYMLINKS = ERRORS_NUMBER_SYMLINKS + 1))
  echo "/etc/apt/apt.conf.d/100update not a symlink"
  echo "Issue command 'ln -s ~/debian-setup/etc/apt/apt.conf.d/100update /etc/apt/apt.conf.d/100update'"
fi
if [[ ! -h ~/.bash_aliases ]]; then
  ((ERRORS_NUMBER_SYMLINKS = ERRORS_NUMBER_SYMLINKS + 1))
  echo "~/.bash_aliases not a symlink." 
  echo "Issue command 'ln -s ~/debian-setup/.bash_aliases ~/.bash_aliases'" 
fi
